...
export function calculateStepRiskScore(step: PlanStep): { score: number; flags: string[] } {
  ...
  switch (step.type) {
    case "FS_READ": {
      score = 5;
      const path = step.inputs.path?.toLowerCase() || "";
    ...
    case "FS_DELETE": {
      score = 55;
      const fileCount = step.inputs.fileCount || 1;
      if (fileCount > 10) {
        score += 20;
    ...
    case "SHELL_RUN": {
      score = 35;
      const cmd = (step.inputs.command + " " + (step.inputs.args || []).join(" ")).toLowerCase();
    ...
    case "NET_ALLOW": {
      score = 15;
      const domains = step.inputs.domains || [];
